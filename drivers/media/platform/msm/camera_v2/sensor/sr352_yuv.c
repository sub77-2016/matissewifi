...
#include "sr352.h"
#if defined (CONFIG_MACH_MILLETWIFIUS_OPEN) || defined (CONFIG_MACH_MILLETLTE_VZW) || defined (CONFIG_MACH_MILLETLTE_ATT) || defined (CONFIG_MACH_DEGASLTE_SPR)
...
#if defined (CONFIG_MACH_MILLETWIFI_OPEN) || defined(CONFIG_MACH_MILLET3G_EUR) || defined(CONFIG_MACH_MATISSE3G_OPEN) || defined(CONFIG_MACH_MATISSEWIFI_OPEN)
...
void sr352_check_hw_revision()
{
    CDBG(" Hardware revision = %d\n",system_rev);
            ...
            if(system_rev >= 7)
                settings_type = 1;
#elif defined (CONFIG_SEC_MATISSE_PROJECT) || defined (CONFIG_MACH_MILLETLTE_ATT) || defined(CONFIG_SEC_DEGAS_PROJECT)
                settings_type = 1;
#endif
}
...
#elif defined(CONFIG_MACH_MILLETWIFI_OPEN) || defined(CONFIG_MACH_MILLET3G_EUR) || defined(CONFIG_MACH_MATISSE3G_OPEN) || defined(CONFIG_MACH_MATISSEWIFI_OPEN)
	...
	void __user *argp)
{
	...
	case CFG_SET_RESOLUTION:
		if(sr352_ctrl.prev_mode == CAMERA_MODE_RECORDING && sr352_ctrl.settings.resolution == MSM_SENSOR_RES_3)
		{
			sr352_init_camera(s_ctrl);
			//Stop stream and start in START_STREAM
			SR352_WRITE_LIST(sr352_stop_stream);
		...
		else if(sr352_ctrl.prev_mode == CAMERA_MODE_RECORDING && sr352_ctrl.settings.resolution != MSM_SENSOR_RES_3)
		{
			if(settings_type == 1) {
