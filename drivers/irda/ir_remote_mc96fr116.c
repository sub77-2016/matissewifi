...
static int irda_fw_update(struct ir_remocon_data *ir_data)
{
	...
	int i, k, ret, checksum, frame_count, start_addr;
	u8 buf_ir_test[8];
	const u8 *IRDA_fw;
	ret = 0;
	...
	if (ret < 0) {
		printk(KERN_CRIT "%s: err %d\n", __func__, ret);
		msleep(50);
		ret = i2c_master_recv(client, buf_ir_test, MC96_READ_LENGTH);
		if (ret < 0) {
			printk(KERN_CRIT "%s: broken FW!\n", __func__);
			goto fix_fw;
		}
	}
#ifdef DEBUG
	...
	if (ret == MC96FR116C_0x103) {
		...
		data->on_off = 0;
		download_pass = 1;
		printk(KERN_CRIT "leaving %s: irda fw fine\n", __func__);
		return 0;
	}
...
fix_fw:
	...
	if (ret == checksum)
		pr_info("%s: Boot mode, First check for FW download clear! ret=%04x\n",
				__func__, ret);
	else {
		pr_err("ABOV IC bootcode broken: First check failed\n");
		goto err_bootmode;
	}

	start_addr = buf_ir_test[2] << 8 | buf_ir_test[3];
	if (start_addr == 0x3FFF)
		pr_info("%s: Boot mode, Second check clear: going for FW download\n", __func__);
	else {
		pr_info("%s: ABOVE IC bootcode broken: Second check failed checksum = %4x\n", __func__, start_addr);
		goto err_bootmode;
	}
	msleep(60);
